...
\centering
\input{plot1.tex}
\caption{Simulation of the case without repulsion for \(n=250\) with \(\rho_{\max}=8\).}
\end{figure}
...
\end{table}
It seems that the number of iterations is quadrupled when the number of mesh points is doubled, indicating that the numbers of iterations runs as \(n^2\). This means that the number of iterations is proportional to the number of elements in the matrix. Note that the matrices handled in this project are tridiagonal, and the trends may be different for dense matrices.
